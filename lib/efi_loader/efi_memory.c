...
//		addr = efi_find_free_memory(len, -1ULL);
		addr = (u64)lmb_alloc_nooverwrite(len, EFI_PAGE_SIZE);
		if (!addr)
			return EFI_OUT_OF_RESOURCES;
		break;
...
//		addr = efi_find_free_memory(len, *memory);
		addr = (u64)lmb_alloc_base_nooverwrite(len, EFI_PAGE_SIZE,
						       *memory);
		if (!addr)
			return EFI_OUT_OF_RESOURCES;
		break;
	case EFI_ALLOCATE_ADDRESS:
		if (*memory & EFI_PAGE_MASK)
			return EFI_NOT_FOUND;
//		addr = *memory;
		addr = (u64)lmb_alloc_addr_nooverwrite(*memory, len);
		if (!addr)
			return EFI_OUT_OF_RESOURCES;
		break;
	default:
...
efi_status_t efi_free_pages(uint64_t memory, efi_uintn_t pages)
{
	...
	status = lmb_free_nooverwrite(memory, pages);
	if (status)
		return EFI_NOT_FOUND;
